...
FC Monitor runs two specialized variants from a single codebase, each optimized for different monitoring needs:

| Variant | URL | Focus |
|---------|-----|-------|
| **🌍 FC Monitor** | [worldmonitor.app](https://worldmonitor.app) | Geopolitical intelligence, military tracking, conflict monitoring, infrastructure security |
...
The Service Status panel tracks the operational health of external services that FCMonitor users may depend on.
...
// App.ts
const header = `FC Monitor v${__APP_VERSION__}`;
```
...
git clone https://github.com/koala73/fcmonitor.git
cd fcmonitor
...
- ✅ **Variant Switcher UI** - Compact orbital navigation between FC Monitor and Tech Monitor
...
2. **Clone your fork** locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/fcmonitor.git
   cd fcmonitor
   ```
